    ...
    component.setData(R"(
        ...
        QtObject {
            ...
            property bool ready: !Sentry.initialized && options.debug && !options.sendClientReports
                ...
                && options.integrations[0].id === "disabled"
...
void SentryQmlUnitTest::reportsIntegrationFlushFailure()
{
    ...
    integration.setId(QStringLiteral("smoke"));
...
void SentryQmlUnitTest::rejectsUnavailableIntegrationService()
{
    ...
    options.setDatabasePath(QDir(temporaryDir.path()).filePath(QStringLiteral("sentry")));
    SentryIntegration integration;
    integration.setId(QStringLiteral("service-smoke"));
#if !defined(SENTRYQML_STATIC)
    ...
    QVERIFY(sentry.close());
    qunsetenv("SENTRY_QML_SERVICE_CONSTRUCTION_MARKER");
}
...
void SentryQmlUnitTest::handlesIntegrationFailures()
{
    ...
    optional.setId(QStringLiteral("missing"));
    ...
    required.setId(QStringLiteral("missing"));
    ...
    duplicateFirst.setId(QStringLiteral("duplicate"));
    duplicateSecond.setId(QStringLiteral("duplicate"));
    ...
    startFailure.setId(QStringLiteral("smoke"));
#if !defined(SENTRYQML_STATIC)
...
void SentryQmlUnitTest::setsLevelAndTransaction()
{
    QTemporaryDir temporaryDir;
    QVERIFY(temporaryDir.isValid());
    ...
    component.setData(R"(
        ...
        QtObject {
            ...
            property bool beforeSendCalled: false
            property bool closed: false
            property string capturedLevel: ""
            property string capturedTransaction: ""
            property string eventId: ""
            property SentryOptions options: SentryOptions {
                databasePath: testDatabasePath
                shutdownTimeout: 2000
                beforeSend: function(event) {
                    capturedLevel = event.level || ""
            ...
            Component.onCompleted: {
                ...
                transactionSet = Sentry.setTransaction("qml-scope-transaction")
                eventId = Sentry.captureMessage("Level and transaction event")
                closed = Sentry.close()
            }
        }
    )", QUrl(QStringLiteral("memory:/SentryLevelAndTransactionTest.qml")));
    ...
    QCOMPARE(object->property("beforeSendCalled").toBool(), true);
    QCOMPARE(object->property("capturedLevel").toString(), QStringLiteral("warning"));
    QCOMPARE(object->property("capturedTransaction").toString(), QStringLiteral("qml-scope-transaction"));
    QCOMPARE(object->property("eventId").toString(), QString());
    QCOMPARE(object->property("closed").toBool(), true);
}
